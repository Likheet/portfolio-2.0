...
import { X, ArrowUpRight, FileText, Quote } from 'lucide-react';
...
const ResearchModal = ({ publication, isOpen, onClose }: ResearchModalProps) => {
                    ...
                    <div ref={contentRef} className="flex flex-col gap-16 md:gap-24">
                        ...
                        <div className="flex flex-col gap-8 text-center items-center">
                            <div className="flex items-center gap-4 text-sm font-mono uppercase tracking-widest text-muted-foreground">
                                <span className="px-3 py-1 border border-foreground/20 rounded-full">{publication.conference}</span>
                            ...
                            {publication.url && publication.url !== '#' && (
                                <Link 
                                    ...
                                    target="_blank"
                                    className="group flex items-center gap-3 px-8 py-4 bg-foreground text-background font-anton text-lg uppercase tracking-wider rounded-full hover:bg-portfolio-red hover:text-white transition-all mt-4"
                                >
                        ...
                        <div className="relative p-8 md:p-16 bg-secondary/5 rounded-3xl border border-foreground/5">
                            ...
                            <div className="relative z-10 max-w-3xl mx-auto text-center flex flex-col gap-6">
                                <h3 className="font-playfair italic text-3xl text-foreground">Abstract</h3>
                        ...
                        {publication.key_contributions && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="md:col-span-2 text-center mb-4">
                                    <h3 className="font-anton text-3xl text-foreground uppercase tracking-wide">Key Contributions</h3>
                                </div>
                                {publication.key_contributions.map((contribution, i) => (
                                    <div key={i} className="flex gap-6 p-8 rounded-2xl border border-foreground/10 hover:border-portfolio-red/50 transition-colors group">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 pt-12 border-t border-foreground/10">
                                {publication.methodology && (
                                    <div className="flex flex-col gap-6">
                                        <h3 className="font-playfair italic text-3xl text-foreground">Methodology</h3>
                                ...
                                {publication.results && (
                                    <div className="flex flex-col gap-6">
                                        <h3 className="font-playfair italic text-3xl text-foreground">Results</h3>
                                        <p className="font-inter text-muted-foreground leading-relaxed">
                                            {publication.results}
                                        </p>
                                    </div>
                        ...
                        {publication.citation && (
                            <div className="flex flex-col gap-4 p-8 bg-secondary/10 rounded-xl font-mono text-sm text-muted-foreground break-all">
                                <span className="uppercase tracking-widest text-xs opacity-50">Citation</span>
